...
const Trapezoid: React.FC<TrapezoidProps> = ({
	...
	stroke = "#2A342A",
	strokeWidth = 2,
	slantedEdges = { top: true, right: false, bottom: true, left: false },
	slantAmount = 20,
}) => {
	const { top, right, bottom, left } = slantedEdges;

	const path = `
        M${left ? slantAmount : 0},0
        H${380 - (right ? slantAmount : 0)}
        ${right ? `L380,${slantAmount}` : ""}
        V${586 - (bottom ? slantAmount : 0)}
        ${bottom ? `L${380 - slantAmount},586` : ""}
        H${left ? slantAmount : 0}
        ${left ? `L0,${586 - slantAmount}` : ""}
        V${top ? slantAmount : 0}
        ${top ? `L${slantAmount},0` : ""}
        Z
    `;
	...
	return (
		<div className={`relative ${className}`}>
			...
			<div className='relative z-10 p-'>{children}</div>
		</div>
	);
};
